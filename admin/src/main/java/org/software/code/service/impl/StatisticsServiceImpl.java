...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public UserStatisticsVo getUserStatistics(String startDate, String endDate) {
        ...
        try {
            ...
            QueryWrapper<User> todayQuery = new QueryWrapper<User>().ge("create_time", today);
            long todayNewUsers = userMapper.selectCount(todayQuery);

            // 4. 查询本月新增用户数
            LocalDateTime monthStart = LocalDateTime.now().withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0).withNano(0);
            QueryWrapper<User> monthQuery = new QueryWrapper<User>().ge("create_time", monthStart);
    ...
    public DeviceStatisticsVo getDeviceStatistics(String startDate, String endDate, String city, Long siteId) {
        ...
        try {
            ...
            if (StrUtil.isNotBlank(city)) {
                // 通过站点表关联查询城市
                QueryWrapper<Site> siteQuery = new QueryWrapper<Site>().eq("city", city);
                // 这里需要联表查询，简化处理
                deviceQuery.in("site_id", siteMapper.selectObjs(siteQuery.select("id")));
            }
            if (siteId != null) {
